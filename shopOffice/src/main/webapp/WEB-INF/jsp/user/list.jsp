...
<script>
	function UserSearch(no){
		$('#currentPage').val(no);
		call_server(searchform,'/getUserList',getlist);
	}
	function getlist(vo){
		list = vo.userlist;
	...
	function memberDetail(id){
		$('#userId').val(id);
		call_server(hiddenform,'/getDetail',getOrderList);		//회원의 주문정보.
	}
		function getOrderList(vo){
			...
			for(var i=0;i<list.length;i++){
				...
				str+="<td>"+"<button type=\"button\" class=\"btn btn-info\" onclick = \'orderPaper();\'>보기</button>"+"</td>";
				str+="</tr>";
